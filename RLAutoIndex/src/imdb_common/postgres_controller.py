...
from dqn_agent.imdb_postgres_schema import PostgresSchema as PostgresDQNSchema
from spg_agent.postgres_converter import PostgresConverter as PostgresSPGConverter
from spg_agent.imdb_postgres_schema import PostgresSchema as PostgresSPGSchema
...
from imdb_workload import IMDBWorkload, sort_queries
from imdb_util import imdb_table_columns
...
class PostgresSystemController(SystemController):
    ...
    def __init__(self, 
        ...
        workload_spec = {
            ...
            "scale_factor": 1, # TODO specify this somewhere, not just in imdb_util 
            "n_selections": n_selections
        }
        self.workload = IMDBWorkload(spec=workload_spec)
    ...
    if not FLAGS.with_agent: 
        ...
        if FLAGS.default_baseline:
            ...
            for tbl in experiment_config['tables']:
                for col in imdb_table_columns[tbl]:
