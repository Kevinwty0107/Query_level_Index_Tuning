...
class ReplayMemory(Memory):
    ...
    def _graph_fn_get_records(self, num_records=1):
        if get_backend() == "tf":
            ...
            indices = tf.compat.v1.random_uniform(shape=(num_records,), maxval=size, dtype=tf.int32)
            indices = (index - 1 - indices) % self.capacity

            # Return default importance weight one.
            return self._read_records(indices=indices), indices, tf.compat.v1.ones_like(tensor=indices, dtype=tf.float32)
        elif get_backend() == "pytorch":
            indices = []
            if self. size > 0:
