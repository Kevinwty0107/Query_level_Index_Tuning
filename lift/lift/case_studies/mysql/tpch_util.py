...
# All table names in tpc-h.
# TPC-H SPEC: http://www.tpc.org/tpc_documents_current_versions/pdf/tpc-h_v2.17.3.pdf
# P. 85ff specify value generation.
tpch_tables = ["NATION", "REGION", "PART", "SUPPLIER", "PARTSUPP", "CUSTOMER", "ORDERS", "LINEITEM"]

SCALE_FACTOR = 1.0

# Maps column type to selection operator.
column_type_operators = {
    int: ["=", "<", ">"],
    float: ["=", "<", ">"],
    str: ["="],
    # No exact date checks.
    "date": ["<", ">"],
    "text": ["LIKE"]
}
...
# Maps table names to names of columns and their type + ranges/values.
tpch_table_columns = {
    "NATION": {
        # Ints have type + range -> range is exclusive of upper bound, so vals 0-24.
        "N_NATIONKEY": [int, "fixed_range", [0, 25]],
        # Lookup means values are fully defined below.
        "N_NAME": [str, "lookup"],
    ...
    "PART": {
        # "scaled_range" -> use TPC-H scale-factor.
        "P_PARTKEY": [int, "scaled_range", [0, 200000]],
        # Sample function defined below to sample hardcoded values.
    ...
    "SUPPLIER": {
        ...
        # Scaled samples need scale factor
...
# Select count(distinct  COLUMN) from LINEITEM;
LINE_ITEM_SELECTIVITY = {
    "L_ORDERKEY": 1.0 / 6001215,
    "L_PARTKEY": 1.0 / 200000,
    "L_SUPPKEY": 1.0 / 10000,
    "L_LINENUMBER": 1.0 / 7,
    "L_QUANTITY": 1.0 / 50.0,
    "L_EXTENDEDPRICE": 1.0 / 933900.0,
    "L_DISCOUNT": 1.0 / 933900.0,
    "L_TAX": 1.0 / 9.0,
    "L_RETURNFLAG": 1.0 / 3.0,
    "L_LINESTATUS": 1.0 / 2.0,
    "L_SHIPDATE": 1.0 / 2526.0,
    "L_COMMITDATE": 1.0 / 2466.0,
    "L_RECEIPTDATE": 1.0 / 2554.0,
    "L_SHIPINSTRUCT": 1.0 / 4.0,
    "L_SHIPMODE": 1.0 / 7.0,
    "L_COMMENT": 1.0 / 4501941
}
TPC_H_SELECTIVITY = {"LINE_ITEM": LINE_ITEM_SELECTIVITY}
...
R_NAME = ['(0, AFRICA)', '(1, AMERICA)', '(2, ASIA)', '(3, EUROPE)', '(4, MIDDLE EAST)']
...
# 4.2.2.14 -> just selected some nouns, verbs etc
...
# Look up pre-defined sample functions from hard-coded values.
